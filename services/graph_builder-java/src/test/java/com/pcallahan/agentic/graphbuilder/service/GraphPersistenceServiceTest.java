...
class GraphPersistenceServiceTest {
    ...
    void persistGraph_ValidGraph_ShouldPersistSuccessfully() {
        ...
        // Create test graph
        Plan plan = new Plan("test_plan", "Test Plan", Path.of("/plans/test_plan"), Set.of("task1"));
        Task task = new Task("task1", "Task 1", Path.of("/tasks/task1"), "test_plan");
        ...
        when(agentGraphRepository.findByTenantIdAndGraphName(tenantId, "test_graph"))
        ...
        // When
        String graphId = graphPersistenceService.persistGraph(graph, tenantId, processId);
        ...
        assertThat(savedGraph.getGraphName()).isEqualTo("test_graph");
        assertThat(savedGraph.getTenantId()).isEqualTo(tenantId);
        assertThat(savedGraph.getProcessId()).isEqualTo(processId);
    }
    ...
    void persistGraph_ExistingGraph_ShouldUpdateGraph() {
        ...
        // Create test graph
        Plan plan = new Plan("test_plan", "Test Plan", Path.of("/plans/test_plan"), Set.of());
        Task task = new Task("task1", "Task 1", Path.of("/tasks/task1"), "test_plan");
        ...
        // Mock existing graph
        AgentGraphEntity existingGraph = new AgentGraphEntity(existingGraphId, tenantId, "test_graph", null);
        when(agentGraphRepository.findByTenantIdAndGraphName(tenantId, "test_graph"))
    ...
    void persistGraph_NullGraph_ShouldThrowException() {
        ...
        // When & Then
        assertThatThrownBy(() -> graphPersistenceService.persistGraph(null, tenantId, processId))
    ...
    void persistGraph_NullTenantId_ShouldThrowException() {
        ...
        // When & Then
        assertThatThrownBy(() -> graphPersistenceService.persistGraph(graph, null, "process-123"))
            .isInstanceOf(GraphPersistenceException.class)
    ...
    void persistGraph_EmptyTenantId_ShouldThrowException() {
        ...
        // When & Then
        assertThatThrownBy(() -> graphPersistenceService.persistGraph(graph, "", "process-123"))
            .isInstanceOf(GraphPersistenceException.class)
            .hasMessageContaining("Tenant ID cannot be null or empty");
    }
    ...
    void persistGraph_RepositoryException_ShouldThrowGraphPersistenceException() {
        ...
        // When & Then
        assertThatThrownBy(() -> graphPersistenceService.persistGraph(graph, tenantId, processId))
            .isInstanceOf(GraphPersistenceException.class)
            .hasMessageContaining("Failed to persist graph");
    }
    ...
    void getPersistedGraph_ExistingGraph_ShouldReturnGraph() {
        ...
        // Mock entities
        AgentGraphEntity graphEntity = new AgentGraphEntity(graphId, tenantId, "test_graph", null);
    ...
    void listPersistedGraphs_ExistingGraphs_ShouldReturnGraphInfoList() {
        ...
        assertThat(result.get(0).getName()).isEqualTo("graph1");
        assertThat(result.get(0).getTenantId()).isEqualTo(tenantId);
        assertThat(result.get(0).getProcessId()).isEqualTo("process1");
        assertThat(result.get(1).getName()).isEqualTo("graph2");
        assertThat(result.get(1).getProcessId()).isEqualTo("process2");
    }
    ...
    private AgentGraphEntity createMockGraphEntity(String id, String name, String tenantId, 
        ...
        AgentGraphEntity entity = new AgentGraphEntity(id, tenantId, name, processId);
